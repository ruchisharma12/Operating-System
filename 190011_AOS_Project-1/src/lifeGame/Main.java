...
public class Main implements Runnable{
	...
	public void run() {
		...
		do {
			...
			try {
				cols = Integer.parseInt(test1);
			} catch (NumberFormatException e) {
				cols = 1;
			}
			String test2 = JOptionPane.showInputDialog("Input number of Rows(y)0 to 130: ");
			try {
				rows = Integer.parseInt(test2);
			} catch (NumberFormatException e) {
				rows = 1;
			}
			String test3 = JOptionPane
					.showInputDialog("Living Condition:eg.23=live if 2/3 neighbors");
			try {
				lc = Integer.parseInt(test3);
			} catch (NumberFormatException e) {
				lc = 23;
			}
			String test4 = JOptionPane.showInputDialog("Birth Condition:eg.345");
			try {
				bc = Integer.parseInt(test4);
			} catch (NumberFormatException e) {
				bc = 3;
			}
		} while (!(cols < 130 && rows < 130 && cols > 0 && rows > 0 && lc >= 0 && bc >= 0 && lc < 87654321
				&& bc < 87654321));
