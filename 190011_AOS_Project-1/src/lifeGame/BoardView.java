	...
	public void paint(Graphics g) {
		...
		for (int y = 0; y < cols; y++) {
			for (int x = 0; x < rows; x++) {
				...
				drawBox(g, 10 + y * innerWidth / cols, 10 + x * innerHeight / rows, 10 + (y + 1) * innerWidth / cols,
						10 + (x + 1) * innerHeight / rows);
				if (model.isAlive(x, y)) {
					...
					g.fillRect(10 + y * innerWidth / cols, 10 + x * innerHeight / rows,
							(int) Math.ceil(innerWidth / cols + 1), (int) Math.ceil(innerHeight / rows + 1));
				}
			}
		}
